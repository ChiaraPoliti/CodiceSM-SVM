...
from SVM_Soft_Margin_Gurobi import SVM_Soft_Margin_Gurobi

"""Funzione che sfrutta SM-SVM (Gurobi) per classificare alcuni dati su 6 database e ritorna i relativi file excel con i risultati delle previsioni"""

#Fisso le variabili su cui farò girare il codice
number_of_tries = 100
training_percentages = np.array([0.75,0.5,0.10])
c_values = [0.5,100]
df_heart_failure = check_database.check_heart()
df_kidney = check_database.check_kidney()
df_rice = check_database.check_rice()
df_breast = check_database.check_breast()
df_spam = check_database.check_spam()
df_eyes = check_database.check_eyes()
    ...
    for C in c_values:
        for training_percentage in training_percentages:
            ...
            for try_number in range(number_of_tries):
                ...
                a, b, z = SVM_Soft_Margin_Gurobi(C, dimensions, observations_training, X_train, y_train)
